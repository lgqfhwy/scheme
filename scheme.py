...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Different number of formal parameter and arguments")
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        #"*** YOUR CODE HERE ***"
        #print("Hevals = ", vals)
        pair_vals = Pair(target.second, vals.second)
        env.define(target.first, do_lambda_form(pair_vals, env))
        return target.first
    else:
...
def do_begin_form(vals, env):
    ...
    for i in range(len(vals) - 1):
        #print("begin,", "len = ", len(vals), "vals = ", vals[i])
...
def check_formals(formals):
    ...
    for e in formals:
        if e in unique_set:
            raise SchemeErro("Symbol ", e, " is repeated")
        if not scheme_symbolp(e):
...
# env = create_global_frame()
# formals, vals = read_line("(a b c)"), read_line("(1 2 3)")
# print(env.make_call_frame(formals, vals))
